...
#ifndef COMBINATORICS_ENUMERATOR_H
#define COMBINATORICS_ENUMERATOR_H
...
namespace combinatorics
{
    ...
    */
    ...
    public:
         ...
         */
        inline
        enumerator(const std::vector<symbol> & p_symbols
         ...
         */
         inline
         void
         set_count(const uint64_t & p_count);
         ...
         inline
         void
         set_word(const unsigned int * const p_word
         ...
         inline
         void
         set_word(const std::string & p_string);
         ...
         inline
         void
         get_word(unsigned int * const p_word
                 ,unsigned int p_word_size
                 ) const;
         ...
         inline
         unsigned int
         get_word_item(unsigned int p_index) const;
          ...
          */
         inline
         void invalidate_root(unsigned int p_nb_items);
         ...
         inline
         int
         compare_word(unsigned int * p_word);
    ...
    ,m_continue(true)
    {
        ...
        {
            throw quicky_exception::quicky_runtime_exception("Word size is greater than total number of symbols"
                                                            ,__LINE__,__FILE__
                                                            );
        }
    ...
    //----------------------------------------------------------------------------
    unsigned int
    enumerator::get_word_size() const
    {
        return m_word_size;
    }
    ...
    //----------------------------------------------------------------------------
    int
    enumerator::compare_word(unsigned int * p_word)
    {
        assert(p_word);
    ...
    //----------------------------------------------------------------------------
    unsigned int
    enumerator::release(unsigned int p_index)
    {
    ...
    //----------------------------------------------------------------------------
    void
    enumerator::take(unsigned int p_index
                    ,unsigned int p_value
                    )
    {
    ...
    //----------------------------------------------------------------------------
    void
    enumerator::set_word(const unsigned int * const p_word
                        ,unsigned int p_word_size
                        )
    {
        ...
        // Copy word
        for(m_index = 0; m_index < p_word_size; ++m_index)
        {
            take(m_index,p_word[m_index]);
        }
        // Initialise remaining part of word
        for(unsigned int l_index = m_index; l_index < m_word_size; ++l_index)
        {
            if(m_word[m_index])
            {
                release(m_index);
    ...
    //----------------------------------------------------------------------------
    void
    enumerator::set_word(const std::string & p_string)
    {
        ...
        // Copy word
        for(m_index = 0; m_index < p_string.size(); ++m_index)
        {
    ...
    //----------------------------------------------------------------------------
    unsigned int
    enumerator::get_word_item(unsigned int p_index) const
    {
        assert(p_index < m_word_size);
    ...
    //----------------------------------------------------------------------------
    void
    enumerator::get_word(unsigned int * const p_word
                        ,unsigned int p_word_size) const
    {
    ...
    //----------------------------------------------------------------------------
    void
    enumerator::invalidate_root(unsigned int p_nb_items)
    {
        assert(p_nb_items);
        assert(p_nb_items <= m_word_size);
        for(unsigned int l_index = p_nb_items; l_index < m_word_size; ++l_index)
        {
            release(l_index);
        }
    ...
    //----------------------------------------------------------------------------
    void
    enumerator::set_count(const uint64_t & p_count)
    {
        m_count = p_count;
    }
    ...
    //----------------------------------------------------------------------------
    unsigned int
    enumerator::get_min_index() const
    {
        return m_min_index;
    }
}
#endif // COMBINATORICS_ENUMERATOR_H
//EOF
